...
import { defineComponent, ref, Ref, toRef, toRefs, watch } from "vue";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    watch(() => route.params, async () => {
      return deferSetup(toRefs(props) as Props, page, route, router);
    });
    return deferSetup(toRefs(props) as Props, page, route, router);
  }
});
...
const deferSetup = (props: Props, page: Ref<string>, route: Route, router: Router) => {
  const config = setDefaultConfig(props)
  ;
  if (!config.pages)
    throw new Error('<markdown-pages>::missing or empty pages attribute')
  ;
  if (!route.params.page) {
    router.push(`${route.path}/${config.pages[0].title}`);
    return { page };
  }

  page.value = config.pages[1].content;
  return { page };
};

const setDefaultConfig = (props: Props) => {
  const defPlaceholder = '#### select an option on the left';
  return {
    pages: props.pages?.value,
    placeholder: props.placeholder?.value ?? defPlaceholder
  };
};
